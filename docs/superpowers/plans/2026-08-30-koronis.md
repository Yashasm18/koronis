...
**Spec:** `docs/superpowers/specs/2026-08-30-koronis-design.md`
...
- **Currency is INR**; all cost figures are declared in `koronis/eval/cost.py` as named constants with a source comment, never inlined.
...
| File | Responsibility |
|---|---|
| `koronis/data/schema.py` | Canonical event column contract, `CampaignSpec` |
| `koronis/data/background.py` | Real transactions → canonical background events |
| `koronis/data/campaigns.py` | Inject labeled campaigns with controlled `(n, k)` |
| `koronis/graph/build.py` | Events → entity-sharing edge lists per relation |
| `koronis/models/velocity.py` | Baseline: per-entity thresholds |
| `koronis/models/gbdt.py` | Baseline: per-transaction LightGBM |
| `koronis/models/layers.py` | From-scratch relational message passing |
| `koronis/models/koronis.py` | Inductive model, feature builder, training loop |
| `koronis/models/loss.py` | Expected-rupee-cost objective |
| `koronis/eval/cost.py` | Cost constants and exposure model |
| `koronis/eval/latency.py` | Replay harness: scores → detection latency |
| `koronis/eval/calibration.py` | Reliability diagram, cost-optimal threshold |
| `koronis/eval/frontier.py` | `(n, k)` sweep and predicted boundary |
| `koronis/cli.py` | Experiment entry points |
...
**Files:**
- Create: `koronis/data/schema.py`
- Create: `koronis/data/background.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.data.background'`

- [ ] **Step 3: Write the schema**

```python
# koronis/data/schema.py
...
```bash
git add koronis/data/schema.py koronis/data/background.py tests/test_background.py
...
**Files:**
- Create: `koronis/data/campaigns.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.data.campaigns'`

- [ ] **Step 3: Write the injector**

```python
# koronis/data/campaigns.py
import numpy as np
import pandas as pd
...
```bash
git add koronis/data/campaigns.py tests/test_campaigns.py
...
**Files:**
- Create: `koronis/models/velocity.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.models.velocity'`

- [ ] **Step 3: Write the detector**

```python
# koronis/models/velocity.py
import numpy as np
import pandas as pd
...
```bash
git add koronis/models/velocity.py tests/test_velocity.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.models.gbdt'`

- [ ] **Step 3: Write the baseline**

```python
# koronis/models/gbdt.py
...
```bash
git add koronis/models/gbdt.py tests/test_gbdt.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.graph.build'`

- [ ] **Step 3: Write the builder**

```python
# koronis/graph/build.py
import numpy as np
import pandas as pd
...
```bash
git add koronis/graph/build.py tests/test_graph.py
...
**Files:**
- Create: `koronis/models/layers.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.models.layers'`
...
```bash
git add koronis/models/layers.py tests/test_layers.py
git commit -m "feat: from-scratch relational message passing with heterophily gate"
```

---

### Task 7: Cost-sensitive loss and the Koronis model

**Files:**
- Create: `koronis/models/loss.py`
- Create: `koronis/models/koronis.py`
- Test: `tests/test_koronis.py`
...
- Produces:
  ...
  - `KoronisDetector(hidden: int = 32, layers: int = 2, window_s: float = 3600.0, seed: int = 0)` with `fit(events, epochs=40) -> None` and `score_events(events) -> np.ndarray`
...
Run: `pytest tests/test_koronis.py -v`
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.models.loss'`

- [ ] **Step 3: Write the loss**

```python
# koronis/models/loss.py
import torch
...
```bash
git add koronis/models/loss.py koronis/models/koronis.py tests/test_koronis.py
...
**Files:**
- Create: `koronis/eval/cost.py`
- Create: `koronis/eval/latency.py`
...
from koronis.data.campaigns import inject
from koronis.data.schema import CampaignSpec
from koronis.eval.latency import detection_times, money_prevented
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.eval.latency'`

- [ ] **Step 3: Write the cost model**

```python
# koronis/eval/cost.py
...
```bash
git add koronis/eval/cost.py koronis/eval/latency.py tests/test_latency.py
...
**Files:**
- Create: `koronis/eval/calibration.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.eval.calibration'`

- [ ] **Step 3: Write the module**

```python
# koronis/eval/calibration.py
import numpy as np
import pandas as pd
...
```bash
git add koronis/eval/calibration.py tests/test_calibration.py
...
**Files:**
- Create: `koronis/eval/frontier.py`
- Test: `tests/test_frontier.py`

**Interfaces:**
- Produces: `predicted_boundary_k(n: int, tau: int) -> float`; `sweep(n_values, k_values, tau, seed) -> pd.DataFrame` with columns `["n", "k", "velocity_detected", "koronis_detected"]`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'koronis.eval.frontier'`

- [ ] **Step 3: Write the module**

```python
# koronis/eval/frontier.py
import numpy as np
import pandas as pd
...
def sweep(n_values: list[int], k_values: list[int], tau: int,
    ...
    for n in n_values:
        for k in k_values:
            ...
            model = KoronisDetector(seed=seed)
            ...
            rows.append({
                ...
                "koronis_detected": bool(rf[y].mean() > rf[~y].mean()),
            })
    return pd.DataFrame(rows)
```
...
```bash
git add koronis/eval/frontier.py tests/test_frontier.py
...
**Files:**
- Create: `koronis/cli.py`
- Create: `README.md`
- Modify: `docs/console-mockup.html` (replace illustrative constants with measured output)

**Interfaces:**
- Consumes: everything above
- Produces: `python -m koronis.cli ablation` and `python -m koronis.cli frontier`, each writing a table to `results/`
...
Run: `python -m koronis.cli ablation`
Expected: a three-row table printed and written to `results/ablation.csv`. The claim under test is that `koronis` has the smallest `detect_s` and the largest `inr_prevented` on the held-out spread campaign.
...
Run: `python -m koronis.cli frontier`
...
```bash
git add koronis/cli.py README.md docs/console-mockup.html results/
...
**Type consistency check.** `score_events(events) -> np.ndarray` is identical across `VelocityDetector`, `GBDTDetector`, `KoronisDetector`. `build_edges` returns a dict keyed by relation name; `KoronisDetector._tensors` orders it by `RELATIONS`, matching `RelationalLayer(n_relations=len(RELATIONS))`. `detection_times` returns `dict[str, float | None]`, and `money_prevented` accepts that `None`. `CampaignSpec` field names are identical in Tasks 1, 2, 10, 11.
